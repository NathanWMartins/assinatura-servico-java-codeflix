...
public class Subscription extends AggregateRoot<SubscriptionId> {
    ...
    public void execute(final SubscriptionCommand... cmds) {
        ...
        for (var cmd : cmds) {
            if (cmd instanceof CancelSubscription) {
                apply((CancelSubscription) cmd);
            } else if (cmd instanceof ChangeStatus) {
                apply((ChangeStatus) cmd);
            } else if (cmd instanceof IncompleteSubscription) {
                apply((IncompleteSubscription) cmd);
            } else if (cmd instanceof RenewSubscription) {
                apply((RenewSubscription) cmd);
            }
        }
